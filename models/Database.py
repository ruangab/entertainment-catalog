...
class Database:
    ...
    @staticmethod
    def _find_one(query, project, collection_name, DB_NAME = os.environ.get("DB_NAME")):
        try:
            collection, connection = Database()._get_collection(collection_name, DB_NAME)
            result = collection.find_one(query, project)
    ...
    @staticmethod
    def _find_all(query, project, collection_name, DB_NAME = os.environ.get("DB_NAME"), limit=False):
        try:
            collection, connection = Database()._get_collection(collection_name, DB_NAME)
            if not limit:
                result = list(collection.find(query, project))
            else:
                result = list(collection.find(query, project).limit(10))
